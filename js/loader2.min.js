...
"-10000px";d.style.display="block";parseInt(document.getElementById(f).clientHeight);d.style.left=c+"px";d.style.display="";d.style.opacity=""};var ShopinpicImage=function(a){this.img=a;this.areas=[];this.promoDiv=this.width=this.height=this.nHeight=this.nWidth=null};ShopinpicImage.prototype.refreshPromo=function(){var a=parseInt(this.img.offsetLeft)+this.img.width-150;this.promoDiv&&(this.promoDiv.style.left=a+"px")};
...
e.minImageHeight&&e.sinpImages.push(a);b--;if(0==b)e.onInitialized()};if(1==c.complete)c.onload()}};Shopinpic.prototype.onInitialized=function(){var a=[],b;for(b in this.sinpImages)this.sinpImages[b].initAreas(),a.push(this.sinpImages[b].img.src);var c=this;this._getDatas(a,function(a){if(a.data)for(var b=0;b<a.data.length;b++){var g=a.data[b];c.sinpImages[g.id].addPromo();c.sinpImages[g.id].initAreas(g.areas)}});if(this.adminMode)for(b in this.sinpImages)this.sinpImages[b].initControlPanel()};
